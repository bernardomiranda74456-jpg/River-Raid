...
PB.Renderer = (function () {
  ...
  const Char = {
    ...
    bone(ctx, P, ax, ay, bx, by, profile, color, opts) {
      ...
      ctx.fillStyle = this.crossGradient(ctx, x1, y1, x2, y2, nx, ny, startW, color);
      ctx.fill(path);
      return path;
    },
    ...
    paddle(ctx, P, elbow, hand, look, side, swinging, prog, readyAmt, serveAmt) {
      ...
      ctx.lineWidth = Math.max(1, 0.012 * u * 2);
      ctx.stroke(grip);
      ctx.fillStyle = '#1b2530';
      ctx.fill(grip);
      ctx.stroke(face);
      const faceGrad = ctx.createLinearGradient(-w, 0, w, 0);
      faceGrad.addColorStop(0, tint(look.paddle, 0.22));
      faceGrad.addColorStop(1, tint(look.paddle, -0.24));
